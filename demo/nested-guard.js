(function(){
  ...
  const BUILD = String(cfg.build || "20260821-0201");
  ...
  function transformHtml(input){
    ...
    if (remaining) {
      throw new Error("TOP Taxi DEMO nested isolation failed: " + remaining);
    }
  ...
  window.fetch = async function(input, init){
    ...
    if (!url || url.origin !== location.origin || /\/demo\//i.test(url.pathname) || !/\.html$/i.test(url.pathname)) {
      return response;
    }
