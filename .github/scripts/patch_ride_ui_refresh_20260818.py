...
# 1) Main title: remove emoji and keep a clean text title. The red accent is added by CSS.
text = text.replace(
...
# 2) Small TOP Taxi wordmark in the card header.
text = text.replace(
...
# 3) Service toggle: no emoji, text-only premium buttons.
text = text.replace('>🚕 一般搭車</button>', '>一般搭車</button>', 1)
text = text.replace('>🚘 代駕服務</button>', '>代駕服務</button>', 1)

# 4) Switching service must not bring the title emoji back.
...
# 5) Add a final override block so we do not disturb existing functional CSS.
marker = '</head>'
style_id = 'topTaxiRideUIRefresh20260818'
if style_id not in text:
    css = r'''
<style id="topTaxiRideUIRefresh20260818">
/* TOP Taxi｜Ride UI Refresh 2026-08-18
   Scope: card title + small wordmark + service selector only.
   Functional controls and ride logic remain unchanged. */
#rideTitle{
  display:flex;
  align-items:center;
  gap:10px;
...
.top-taxi-mini-logo span{
  color:var(--red);
}

/* Figure 2 direction: soft elevated premium segmented control, no emoji. */
.ride-service-switch{
...
# Guard rails: the visible service selector and dynamic title must be emoji-free.
required = [
    '<div id="rideTitle" class="form-title">即時叫車</div>',
    '<div class="form-label-small top-taxi-mini-logo"><span>TOP</span> Taxi</div>',
    '>一般搭車</button>',
...
print('PASS: ride UI refresh patch applied')
