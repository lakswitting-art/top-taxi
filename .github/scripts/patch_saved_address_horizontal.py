...
MARKER = "topTaxiSavedAddressNoHorizontalDragV1"

RIDE_CSS = r'''
<style id="topTaxiSavedAddressNoHorizontalDragV1">
/* Saved-address editor only: prevent Google suggestions from widening/panning the page. */
...
#settings.ride-saved-compact .autocomplete { width: 100%; }
#settings.ride-saved-compact .suggestions {
  left: 0;
  right: 0;
  width: auto;
...
#settings.ride-saved-compact .suggestion { overflow: hidden; }
...
#bookingSavedAddressEditor .address-autocomplete { width: 100%; }
#bookingSavedAddressEditor .address-suggestions {
  left: 0;
  right: 0;
  width: auto;
...
#bookingSavedAddressEditor .address-suggestion { overflow: hidden; }
...
def patch(filename, css, required):
    ...
    if MARKER in text:
        raise SystemExit(f"{filename}: patch marker already exists")
    if "</head>" not in text:
...
for f in ["ride.html", "index.html"]:
    ...
    assert t.count(MARKER) == 1, f
    assert "touch-action: pan-y" in t, f

print("Patched ride.html + index.html only; fare/errand untouched")
